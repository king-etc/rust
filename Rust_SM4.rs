use std::mem;
pub const BLOCK_SIZE: usize = 16;
pub const KEY_SIZE: usize = 16;
pub const SM4_KEY_SCHEDULE: usize = 32;
pub const RND_KEY_SIZE: usize = 32;

pub type Key = [u32; SM4_KEY_SCHEDULE];
pub type Block = [u8; BLOCK_SIZE];
pub type block128_f = fn(&Block,&Key)->Block;
const SM4_S:[u8;256]=
[
    
];

const SM4_SBOX_T:[u32;256]=
[
    0x8ED55B5B, 0xD0924242, 0x4DEAA7A7, 0x06FDFBFB, 0xFCCF3333, 0x65E28787,
    0xC93DF4F4, 0x6BB5DEDE, 0x4E165858, 0x6EB4DADA, 0x44145050, 0xCAC10B0B,
    0x8828A0A0, 0x17F8EFEF, 0x9C2CB0B0, 0x11051414, 0x872BACAC, 0xFB669D9D,
    0xF2986A6A, 0xAE77D9D9, 0x822AA8A8, 0x46BCFAFA, 0x14041010, 0xCFC00F0F,
    0x02A8AAAA, 0x54451111, 0x5F134C4C, 0xBE269898, 0x6D482525, 0x9E841A1A,
    0x1E061818, 0xFD9B6666, 0xEC9E7272, 0x4A430909, 0x10514141, 0x24F7D3D3,
    0xD5934646, 0x53ECBFBF, 0xF89A6262, 0x927BE9E9, 0xFF33CCCC, 0x04555151,
    0x270B2C2C, 0x4F420D0D, 0x59EEB7B7, 0xF3CC3F3F, 0x1CAEB2B2, 0xEA638989,
    0x74E79393, 0x7FB1CECE, 0x6C1C7070, 0x0DABA6A6, 0xEDCA2727, 0x28082020,
    0x48EBA3A3, 0xC1975656, 0x80820202, 0xA3DC7F7F, 0xC4965252, 0x12F9EBEB,
    0xA174D5D5, 0xB38D3E3E, 0xC33FFCFC, 0x3EA49A9A, 0x5B461D1D, 0x1B071C1C,
    0x3BA59E9E, 0x0CFFF3F3, 0x3FF0CFCF, 0xBF72CDCD, 0x4B175C5C, 0x52B8EAEA,
    0x8F810E0E, 0x3D586565, 0xCC3CF0F0, 0x7D196464, 0x7EE59B9B, 0x91871616,
    0x734E3D3D, 0x08AAA2A2, 0xC869A1A1, 0xC76AADAD, 0x85830606, 0x7AB0CACA,
    0xB570C5C5, 0xF4659191, 0xB2D96B6B, 0xA7892E2E, 0x18FBE3E3, 0x47E8AFAF,
    0x330F3C3C, 0x674A2D2D, 0xB071C1C1, 0x0E575959, 0xE99F7676, 0xE135D4D4,
    0x661E7878, 0xB4249090, 0x360E3838, 0x265F7979, 0xEF628D8D, 0x38596161,
    0x95D24747, 0x2AA08A8A, 0xB1259494, 0xAA228888, 0x8C7DF1F1, 0xD73BECEC,
    0x05010404, 0xA5218484, 0x9879E1E1, 0x9B851E1E, 0x84D75353, 0x00000000,
    0x5E471919, 0x0B565D5D, 0xE39D7E7E, 0x9FD04F4F, 0xBB279C9C, 0x1A534949,
    0x7C4D3131, 0xEE36D8D8, 0x0A020808, 0x7BE49F9F, 0x20A28282, 0xD4C71313,
    0xE8CB2323, 0xE69C7A7A, 0x42E9ABAB, 0x43BDFEFE, 0xA2882A2A, 0x9AD14B4B,
    0x40410101, 0xDBC41F1F, 0xD838E0E0, 0x61B7D6D6, 0x2FA18E8E, 0x2BF4DFDF,
    0x3AF1CBCB, 0xF6CD3B3B, 0x1DFAE7E7, 0xE5608585, 0x41155454, 0x25A38686,
    0x60E38383, 0x16ACBABA, 0x295C7575, 0x34A69292, 0xF7996E6E, 0xE434D0D0,
    0x721A6868, 0x01545555, 0x19AFB6B6, 0xDF914E4E, 0xFA32C8C8, 0xF030C0C0,
    0x21F6D7D7, 0xBC8E3232, 0x75B3C6C6, 0x6FE08F8F, 0x691D7474, 0x2EF5DBDB,
    0x6AE18B8B, 0x962EB8B8, 0x8A800A0A, 0xFE679999, 0xE2C92B2B, 0xE0618181,
    0xC0C30303, 0x8D29A4A4, 0xAF238C8C, 0x07A9AEAE, 0x390D3434, 0x1F524D4D,
    0x764F3939, 0xD36EBDBD, 0x81D65757, 0xB7D86F6F, 0xEB37DCDC, 0x51441515,
    0xA6DD7B7B, 0x09FEF7F7, 0xB68C3A3A, 0x932FBCBC, 0x0F030C0C, 0x03FCFFFF,
    0xC26BA9A9, 0xBA73C9C9, 0xD96CB5B5, 0xDC6DB1B1, 0x375A6D6D, 0x15504545,
    0xB98F3636, 0x771B6C6C, 0x13ADBEBE, 0xDA904A4A, 0x57B9EEEE, 0xA9DE7777,
    0x4CBEF2F2, 0x837EFDFD, 0x55114444, 0xBDDA6767, 0x2C5D7171, 0x45400505,
    0x631F7C7C, 0x50104040, 0x325B6969, 0xB8DB6363, 0x220A2828, 0xC5C20707,
    0xF531C4C4, 0xA88A2222, 0x31A79696, 0xF9CE3737, 0x977AEDED, 0x49BFF6F6,
    0x992DB4B4, 0xA475D1D1, 0x90D34343, 0x5A124848, 0x58BAE2E2, 0x71E69797,
    0x64B6D2D2, 0x70B2C2C2, 0xAD8B2626, 0xCD68A5A5, 0xCB955E5E, 0x624B2929,
    0x3C0C3030, 0xCE945A5A, 0xAB76DDDD, 0x867FF9F9, 0xF1649595, 0x5DBBE6E6,
    0x35F2C7C7, 0x2D092424, 0xD1C61717, 0xD66FB9B9, 0xDEC51B1B, 0x94861212,
    0x78186060, 0x30F3C3C3, 0x897CF5F5, 0x5CEFB3B3, 0xD23AE8E8, 0xACDF7373,
    0x794C3535, 0xA0208080, 0x9D78E5E5, 0x56EDBBBB, 0x235E7D7D, 0xC63EF8F8,
    0x8BD45F5F, 0xE7C82F2F, 0xDD39E4E4, 0x68492121
];
const FK:[u32;4]=
[
    0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc
];
const CK:[u32;32]=
[
    0x00070E15, 0x1C232A31, 0x383F464D, 0x545B6269,
    0x70777E85, 0x8C939AA1, 0xA8AFB6BD, 0xC4CBD2D9,
    0xE0E7EEF5, 0xFC030A11, 0x181F262D, 0x343B4249,
    0x50575E65, 0x6C737A81, 0x888F969D, 0xA4ABB2B9,
    0xC0C7CED5, 0xDCE3EAF1, 0xF8FF060D, 0x141B2229,
    0x30373E45, 0x4C535A61, 0x686F767D, 0x848B9299,
    0xA0A7AEB5, 0xBCC3CAD1, 0xD8DFE6ED, 0xF4FB0209,
    0x10171E25, 0x2C333A41, 0x484F565D, 0x646B7279
];
fn rotl(a:u32,n:u8)->u32
{
    return (a<<n) | (a>>(32-n));
}

fn load_u32_be(b:&[u8],n:u32) -> u32
{
    return ((b[(4*n) as usize] as u32) <<24) 
    |   ((b[(4*n+1) as usize] as u32)<<16) 
    |   ((b[(4*n+2) as usize] as u32)<<8)
    |   (b[(4*n+3) as usize] as u32);
}
fn store_u32_be(v:u32,b:&mut [u8]) -> ()
{
    b[0]=(v>>24)as u8;
    b[1]=(v>>16)as u8;
    b[2]=(v>>8) as u8;
    b[3]=(v)    as u8;
}
fn SM4_T_slow(X:u32)-> u32
{
    let mut t:u32 = 0;
    t |= (SM4_S[((X >> 24)  as u8)  as usize] as u32) <<24;
    t |= (SM4_S[((X >> 16)  as u8)  as usize] as u32) <<16;
    t |= (SM4_S[((X >> 8)   as u8)  as usize] as u32) <<8;
    t |= SM4_S[(X as u8)            as usize] as u32;
    return t ^ rotl(t, 2) ^ rotl(t, 10) ^ rotl(t, 18) ^ rotl(t, 24);
}
fn SM4_T(X:u32)->u32
{
    return 
    SM4_SBOX_T[((X >> 24)       as u8) as usize] ^
    rotl(SM4_SBOX_T[((X >> 16)  as u8) as usize], 24) ^
    rotl(SM4_SBOX_T[((X >> 8)   as u8) as usize], 16) ^
    rotl(SM4_SBOX_T[(X as u8)          as usize], 8);
}


macro_rules! SM4_RNDS 
{
    (
        $k0:expr,
        $k1:expr,
        $k2:expr,
        $k3:expr,
        $B0:ident,
        $B1:ident,
        $B2:ident,
        $B3:ident,
        $ks:ident,
        $F:ident
    ) => 
    {
        $B0 ^= $F($B1 ^ $B2 ^ $B3 ^ $ks[$k0]); 
        $B1 ^= $F($B0 ^ $B2 ^ $B3 ^ $ks[$k1]); 
        $B2 ^= $F($B0 ^ $B1 ^ $B3 ^ $ks[$k2]); 
        $B3 ^= $F($B0 ^ $B1 ^ $B2 ^ $ks[$k3]); 
    };
}
fn SM4_set_key(key:&[u8;32],ks:&mut Key)-> i32
{
    let mut K:[u32;4]=[0;4];
    let mut i=0;
    K[0] = load_u32_be(key, 0) ^ FK[0];
    K[1] = load_u32_be(key, 1) ^ FK[1];
    K[2] = load_u32_be(key, 2) ^ FK[2];
    K[3] = load_u32_be(key, 3) ^ FK[3];
    for i in 0..32
    {
        let X = K[(i + 1) % 4] ^ K[(i + 2) % 4] ^ K[(i + 3) % 4] ^ CK[i];
        let mut t = 0;
        t |= ( u32::from(SM4_S[((X >> 24) as u8) as usize])) << 24;
        t |= ( u32::from(SM4_S[((X >> 16) as u8) as usize])) << 16;
        t |= ( u32::from(SM4_S[((X >> 8) as u8) as usize])) << 8;
        t |= ( u32::from(SM4_S[(X as u8) as usize]));
        t = t ^ rotl(t, 13) ^ rotl(t, 23);
        K[i % 4] ^= t;
        ks[i] = K[i % 4];
    }
    return 1;
}

fn SM4_encrypt(InData:& Block,ks:& Key) -> Block
{
    let mut B0:u32 = load_u32_be(InData, 0);
    let mut B1:u32 = load_u32_be(InData, 1);
    let mut B2:u32 = load_u32_be(InData, 2);
    let mut B3:u32 = load_u32_be(InData, 3);
    SM4_RNDS!(0,1,2,3, B0,B1,B2,B3, ks, SM4_T_slow);
    SM4_RNDS!(4, 5, 6, 7, B0,B1,B2,B3, ks, SM4_T);
    SM4_RNDS!(8, 9, 10, 11, B0,B1,B2,B3, ks, SM4_T);
    SM4_RNDS!(12, 13, 14, 15, B0,B1,B2,B3, ks, SM4_T);
    SM4_RNDS!(16, 17, 18, 19, B0,B1,B2,B3, ks, SM4_T);
    SM4_RNDS!(20, 21, 22, 23, B0,B1,B2,B3, ks, SM4_T);
    SM4_RNDS!(24, 25, 26, 27, B0,B1,B2,B3, ks, SM4_T);
    SM4_RNDS!(28, 29, 30, 31, B0,B1,B2,B3, ks, SM4_T_slow);
    
    let mut OutData:Block =[0;BLOCK_SIZE];
    store_u32_be(B3,&mut OutData[0 .. 4]);
    store_u32_be(B2, &mut OutData[4 .. 8]);
    store_u32_be(B1, &mut OutData[8 .. 12]);
    store_u32_be(B0, &mut OutData[12 .. 16]);
    OutData
}

fn SM4_decrypt(InData:& Block, ks:&Key) -> Block
{
    let mut B0:u32 = load_u32_be(InData, 0);
    let mut B1:u32 = load_u32_be(InData, 1);
    let mut B2:u32 = load_u32_be(InData, 2);
    let mut B3:u32 = load_u32_be(InData, 3);

    SM4_RNDS!(31, 30, 29, 28, B0,B1,B2,B3, ks, SM4_T_slow);
    SM4_RNDS!(27, 26, 25, 24, B0,B1,B2,B3, ks, SM4_T);
    SM4_RNDS!(23, 22, 21, 20, B0,B1,B2,B3, ks, SM4_T);
    SM4_RNDS!(19, 18, 17, 16, B0,B1,B2,B3, ks, SM4_T);
    SM4_RNDS!(15, 14, 13, 12, B0,B1,B2,B3, ks, SM4_T);
    SM4_RNDS!(11, 10,  9,  8, B0,B1,B2,B3, ks, SM4_T);
    SM4_RNDS!( 7,  6,  5,  4, B0,B1,B2,B3, ks, SM4_T);
    SM4_RNDS!( 3,  2,  1,  0, B0,B1,B2,B3, ks, SM4_T_slow);
    
    let mut OutData:Block =[0;BLOCK_SIZE];
    store_u32_be(B3, &mut OutData[0 .. 4]);
    store_u32_be(B2, &mut OutData[4 .. 8]);
    store_u32_be(B1, &mut OutData[8 .. 12]);
    store_u32_be(B0, &mut OutData[12 .. 16]);
    OutData 
}


#[inline(always)]
pub fn xor(buf: &mut [u8], key: &[u8]) 
{
    //debug_assert_eq!(buf.len(), key.len());
    for (a, b) in buf.iter_mut().zip(key) {
        *a ^= *b;
    }
}

fn CRYPTO_cbc128_encrypt(In:&[u8],Out: & mut  Vec<u8>,Len:usize,Ks:&Key,Iv:& Block,block:block128_f)->()
{
    if Len==0
    {
        return;
    }
    else
    {
        let per:usize = Len/16;
        let mut block_s : Block =[0;BLOCK_SIZE];
        let mut IV_data:Block=[0;BLOCK_SIZE];
        IV_data.copy_from_slice(&Iv[0..]);
        for n in 0..per
        {

            block_s.copy_from_slice(&In[n*BLOCK_SIZE..(n+1)*BLOCK_SIZE]);
            xor(& mut block_s,&mut IV_data[0..]);
            IV_data.copy_from_slice(&block(&block_s,Ks)) ;
            Out.extend(IV_data.iter().cloned()) ;
        }
        if Len%BLOCK_SIZE !=0
        {
            block_s=[0;BLOCK_SIZE];
            block_s.copy_from_slice(&In[per*BLOCK_SIZE..]);
            xor(& mut block_s,&IV_data[0..]);
            Out.extend(block(&block_s,Ks).iter().cloned()) ;
            IV_data.copy_from_slice(&block_s);
        }
    }
}
fn CRYPTO_cbc128_decrypt(In:&[u8],Out:& mut Vec<u8>,Len:usize,Ks:&Key,Iv:&  Block,block:block128_f)->()
{
    if Len==0
    {
        return;
    }
    else
    {
        let per:usize = Len/16;
        let mut block_s : Block =[0;BLOCK_SIZE];
        let mut IV_data:Block=[0;BLOCK_SIZE];
        let mut block_cop : Block =[0;BLOCK_SIZE];
        IV_data.copy_from_slice(&Iv[0..]);
        for n in 0..per
        {

            block_s.copy_from_slice(&In[n*BLOCK_SIZE..(n+1)*BLOCK_SIZE]);
            block_cop.copy_from_slice(&block(&block_s,Ks));
            xor(& mut block_cop,&IV_data[0..]);
            Out.extend(block_cop.iter().cloned()) ;
            IV_data.copy_from_slice(&block_s);
        }
    }
}
